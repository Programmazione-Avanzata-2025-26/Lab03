...
class Autonoleggio:
    ...
    def aggiungi_automobile(self, marca, modello, anno, num_posti):
        """Aggiunge un'automobile nell'autonoleggio: aggiunge solo nel sistema e non aggiorna il file"""
                if self.automobili:
            ultimi_codici = [int(a.codice[1:]) for a in self.automobili]
            '''
            # La riga in alto è equivalente a scrivere
    ...
    def automobili_ordinate_per_marca(self):
        ...
        # SOLUZIONE 1 ####################################################################
        lista_tuple = [] # inizializza lista di tuple (marca, oggetto_automobile)
        for a in self.automobili:
            lista_tuple.append((a.marca, a)) # aggiunge la tupla (marca, oggetto_automobile)

        lista_tuple.sort() # ordina per il primo elemento della tupla (marca)

        lista_ordinata = [] # inizializzo la lista di auto ordinate
        for t in lista_tuple:
            lista_ordinata.append(t[1]) # prendo solo l'oggetto della tupla (alla posizione 1) e lo aggiungo alla lista
        return lista_ordinata

        # SOLUZIONE 2 (sfrutta il concetto di classe) ##################################
        # NB: bisogna importare attrgetter con "from operator import attrgetter"
        #
